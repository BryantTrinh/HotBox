...
DEFAULT_PRIZES = {
    ...
    "golden_mystery_bag": {
        "name": "✨💰 Golden Mystery Bag",
        "rarity": "Secret Rare",
        "color": 0xF7C843,
        "chance": 3,
        "flavor": "✨ *The Golden Mystery Bag radiates a blinding glow…*",
...
class DealOrNoDeal(commands.Cog):
    ...
    def _can_user_win_now(self, user_id: int) -> bool:
        """
        Users cannot win back-to-back unless their previous prize
        was Bit Roulette or Bit Frame.
        """
        if not self.winners:
            return True
        ...
        if last_prize_key in ("bit_roulette", "bit_frame"):
            return True
    ...
    async def _send_box_message(self, bypass_daily_limit: bool = False):
        ...
        while True:
            ...
            if self._can_user_win_now(user.id):
                ...
                try:
                    await channel.send(
                        f"{user.mention}, you cannot win back-to-back unless "
                        f"your last prize was **Bit Roulette** or **Bit Frame**."
                    )
                except Exception:
                    pass
                continue
    ...
    async def deal_help(self, ctx):
        embed = discord.Embed(
            ...
            description=(
                "**Concept of the Game:**\n"
                "A random **mystery box** appears every **1–5 minutes** while the "
        ...
        embed.add_field(
            ...
            value=(
                ...
                "• You cannot win back-to-back **unless** your last prize was:\n"
                "   → **Bit Roulette**\n"
                "   → **Bit Frame**\n"
        ...
        # 🔥 Add Bry Signature at the VERY bottom under all fields+footer
        embed.add_field(
            name="\u200b",  # keeps field label invisible
    ...
    async def deal_day_reset(self, ctx):
        ...
        - Schedules next spawn 1–5 minutes from now
        """
        if ctx.author.id not in WHITELIST:
