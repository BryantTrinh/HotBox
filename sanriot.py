...
# -----------------------------
    ...
    async def battle(self, ctx, opponent: discord.Member):
        ...
        try:
            ...
            async def select_character(user_id, user_obj):
                ...
                class CharSelect(discord.ui.Select):
                    def __init__(self, parent_view):
                        ...
                        super().__init__(
                            placeholder=f"Select your character, {user_obj.display_name}",
                            options=options,
                            min_values=1,
                            max_values=1
                        )
                ...
                embed = discord.Embed(
                    ...
                    description=f"Only {user_obj.mention} can use this menu.",
                    color=discord.Color.blurple()
                )
            ...
            try:
                ...
                challenger_char, opponent_char = await asyncio.gather(challenger_task, opponent_task)
            ...
            while c_hp > 0 and o_hp > 0:
                ...
                dodge_chance = defender["stats"].get("dodge", 0)

                if random.randint(1, 100) <= dodge_chance * 2:
                    attack_text = f"💨 {defender_user.display_name}'s {defender['name']} (#{defender['id']}) dodged {attacker_user.display_name}'s attack!"
                else:
                    if turn % 2 == 0:
                        o_hp -= dmg
                    else:
                        c_hp -= dmg
                    attack_text = f"💥 {attacker_user.display_name}'s {attacker['name']} hits {defender_user.display_name}'s {defender['name']} for {dmg} damage!"

                log.append(attack_text)

                battle_embed.set_field_at(
                    0, name=f"{ctx.author.display_name}'s {challenger_char['name']} (#{challenger_char['id']})",
                    value=hp_bar(c_hp, c_max_hp), inline=True
                )
                battle_embed.set_field_at(
                    1, name=f"{opponent.display_name}'s {opponent_char['name']} (#{opponent_char['id']})",
                    value=hp_bar(o_hp, o_max_hp), inline=True
                )
            ...
            log.append(f"🏆 {ctx.guild.get_member(int(winner_id)).display_name}'s {winner_char['name']} wins!")
            log.append(f"🎁 {ctx.guild.get_member(int(winner_id)).display_name} takes {loser_char['name']} from {ctx.guild.get_member(int(loser_id)).display_name}!")
    ...
    async def date(self, ctx):
        user_id = str(ctx.author.id)
        async with self.data_lock:
            player = self.data["players"].setdefault(user_id, {"characters": [], "date_points": 0})
            if not player.get("characters"):
        ...
        BASE_QUESTIONS = [
            ("Where should we go today?", [
                "🎢 Amusement Park", "🍽 Restaurant", "🌳 Picnic", "🏠 Stay Home"
            ]),
            ("What should we eat?", [
                "🍎 Fruit", "🍕 Pizza", "🍞 Bread", "🥗 Salad"
            ]),
            ("What should we do after dinner?", [
                "🎤 Karaoke", "🎬 Movie", "🎮 Play Games", "📖 Read"
            ]),
            ("How do you say goodbye?", [
                "💐 Flowers", "🤝 Handshake", "👋 Wave", "👍 Fist Bump"
            ]),
        ]

        def get_randomized_questions():
            randomized_questions = []
            for question, options in BASE_QUESTIONS:
                randomized_options = [(opt, random.randint(0, 3)) for opt in options]
                randomized_questions.append((question, randomized_options))
        ...
        # Dynamic results
        GREAT_RESULTS = [
            f"💖 {char['name']} had an amazing time with you! They can’t stop smiling!",
            f"🌟 {char['name']} says it was one of the best days ever!",
            f"💞 {char['name']} looks at you with sparkly eyes... 'Let’s do this again soon!'",
        ]

        GOOD_RESULTS = [
            f"🌸 {char['name']} giggles softly, 'I had such a lovely time with you~'",
            f"💐 {char['name']} says, 'That was nice, I had fun!'",
            f"🎀 {char['name']} blushes a little, 'You’re pretty fun to hang out with!'",
        ]

        BAD_RESULTS = [
            f"😅 {char['name']} felt the date was a bit awkward... maybe next time?",
            f"😕 {char['name']} looks unsure. 'Maybe we just didn’t click today.'",
            f"🙈 {char['name']} forces a smile, 'Let’s just pretend that went well, okay?'",
        ]

        if total_score >= 10:
            result = random.choice(GREAT_RESULTS)
        elif total_score >= 6:
            result = random.choice(GOOD_RESULTS)
        else:
            result = random.choice(BAD_RESULTS)
...
# Date Shop Command
# -----------------------------
    @commands.command(name="dateshop")
    async def dateshop(self, ctx):
        user_id = str(ctx.author.id)
        ...
        class ShopSelect(discord.ui.Select):
            def __init__(self):
                options = [
                    discord.SelectOption(
                        label=f"{name} - {data['cost']} Date Points",
                        description=data['desc'],
                        value=name
                    ) for name, data in SHOP_ITEMS.items()
                ]
                super().__init__(placeholder="Select an item to purchase...", options=options, min_values=1, max_values=1)
        ...
        class ShopView(discord.ui.View):
            def __init__(self):
                ...
                self.select_item = ShopSelect()
                self.add_item(self.select_item)

        shop_view = ShopView()
        shop_msg = await ctx.send(f"💎 You have **{player['date_points']} Date Points**.\nSelect an item to purchase:", view=shop_view)
        await shop_view.wait()

        if not shop_view.select_item.selected_item:
            return await ctx.send("❌ You didn’t select an item.")

        item_name = shop_view.select_item.selected_item
        item_data = SHOP_ITEMS[item_name]

        if player["date_points"] < item_data["cost"]:
            return await ctx.send(f"❌ Not enough Date Points. You need {item_data['cost']}.")

        # -----------------------------
        # Character selection for items
        # -----------------------------
        class CharSelect(discord.ui.Select):
            def __init__(self, characters):
                options = [
                    discord.SelectOption(
                        label=f"{c['name']} (#{c['id']})",
                        description=", ".join(f"{k}: {v}" for k, v in c['stats'].items()),
                        value=str(i)
                    ) for i, c in enumerate(characters)
                ]
                super().__init__(placeholder="Select a character to apply this item...", options=options)
                self.selected_index = None
        ...
        class CharView(discord.ui.View):
            def __init__(self, characters):
                super().__init__(timeout=60)
                self.select_item = CharSelect(characters)
                self.add_item(self.select_item)

        char_view = CharView(player["characters"])
        char_msg = await ctx.send(f"Select a character to apply **{item_name}** to:", view=char_view)
        await char_view.wait()

        if char_view.select_item.selected_index is None:
            return await ctx.send("❌ No character selected.")

        char = player["characters"][char_view.select_item.selected_index]

        # -----------------------------
        # Apply item effects
        # -----------------------------
        if item_name == "Gift Box":
            stat = random.choice(["strength", "agility", "dodge", "hp"])
            increase = random.randint(1, 2)
            char["stats"][stat] += increase
            effect_text = f"💖 {stat.title()} increased by {increase}!"

        elif item_name == "Outfit Upgrade":
            stat = random.choice(["strength", "agility", "dodge", "hp"])
            increase = random.randint(1, 2)
            char["stats"][stat] += increase
            effect_text = f"👗 {stat.title()} increased by {increase}!"

        elif item_name == "Golden Date Ticket":
            increases = {}
            for stat in ["strength", "agility", "dodge", "hp"]:
        ...
        # -----------------------------
        embed = discord.Embed(
            title=f"🛍️ Purchased {item_name}!",
            description=f"You applied **{item_name}** to **{char['name']} (#{char['id']})**.\n\n{effect_text}\n\n💎 Remaining Date Points: {player['date_points']}",
