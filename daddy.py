...
LUV_USER_ID = {1370076515429253264, 821757719207739412}
...
PAT_GIFS = [
    ...
    "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExb3VtMTZzNGgxbHJ5czhnd2t6d21jam92b2p5d2o3cDZxeHZsZm1hNyZlcD12MV9naWZzX3NlYXJjaCZjdD1n/QgtX0RS7ObVoXEnsBl/giphy.gif",
...
class LunaDaughter(commands.Cog):
    ...
    def get_affection(self, user_id: int, is_luv: bool) -> int:
        if user_id not in self.affection:
            self.affection[user_id] = -20 if is_luv else 50
    ...
    def set_affection(self, user_id: int, value: int, is_luv: bool) -> None:
        if is_luv:
            ...
            self.affection[user_id] = max(50, min(100, value))  # never below 50

        else:

            self.affection[user_id] = max(-50, min(100, value))
    ...
    async def start_conversation(self, ctx: commands.Context):
        ...
        if not is_luv:
            if stage == "shy":
                lines.append("U-um... hi darling. I was waiting quietly for you...")
            elif stage == "soft":
                lines.append("Hehe, you're back~ Pick something and talk to me, okay?")
            elif stage == "clingy":
                lines.append("Babe! You're finally here. I'm not letting you go easily this time.")
            else:
                lines.append("Honey, you're *mine* for a bit now. Choose carefully, okay?")
        else:
            lines.append("...Oh. It's you.")
            lines.append("Why are you here now, when Bry and I were fine without you?")
...
GUILT_SPIRAL_LINES = [
    ...
    "If you ever feel trapped, I promise I only meant to feel safe next to you.",
...
DONT_LEAVE_ME_LINES = [
    ...
    "I panic at the thought of you choosing distance.",
